...
-- changeset kate:14
DROP TABLE IF EXISTS delete_skill CASCADE;
...
CREATE OR REPLACE FUNCTION save_skill()
    RETURNS TRIGGER AS $$
BEGIN
    INSERT INTO delete_skill VALUES (OLD.skill_id, OLD.skill, CURRENT_DATE);
    RETURN OLD;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER delete_skill_trigger
    AFTER DELETE
    ON skill_entity
    FOR EACH ROW
EXECUTE FUNCTION save_skill();